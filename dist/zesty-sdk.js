...
// ─── Factory ─────────────────────────────────────────────────────────────────
export function createZestySdk(config = {}) {
    const { authToken: configAuthToken, sessionToken: configSessionToken, instanceZuid: configInstanceZuid, accountsBaseUrl = "https://accounts.api.zesty.io/v1", authBaseUrl = "https://auth.api.zesty.io", metricsBaseUrl = "https://metrics.zesty.io", mediaManagerBaseUrl = "https://media-manager.api.zesty.io", mediaStorageBaseUrl = "https://media-storage.api.zesty.io", accountsBaseUrlOverride, contentBaseUrlOverride, mediaBaseUrlOverride, onError, fetch: configFetch, } = config;
    ...
    function resolveAuth(explicitToken) {
        ...
        if (configAuthToken)
            return `Bearer ${configAuthToken}`;
        if (configSessionToken)
            return `Bearer ${configSessionToken}`;
    ...
    async function request(method, url, opts = {}) {
        ...
        try {
            ...
            const err = new ZestyApiError(`Network error: ${String(e)}`, 0, method, fullUrl);
            onError?.(err);
            throw err;
        }
        ...
        if (!res.ok) {
            ...
            const p = parsed;
            const message = (typeof p?.error === "string" ? p.error : null) ??
    ...
    function resolveInstance(provided) {
        ...
        if (!z)
            throw new Error("instanceZuid is required — pass it as an argument or set it in config");
        return z;
    }
    // ─── Namespaces ───────────────────────────────────────────────────────────────
    // ── auth ──────────────────────────────────────────────────────────────────────
    ...
    const media = {
        ...
        async upload(bucketName, file, opts) {
            ...
            const result = await request("POST", mediaStorageUrl(`/upload/gcp/${bucketName}`), { formData });
...
//# sourceMappingURL=zesty-sdk.js.map